...
Shell.prototype._exec = function(command, options, cb) {
  options = options || {};
  ...
  if (cb == null || cb == undefined)
    exec(command, extend({ maxBuffer: 1000 * 1024 }, options.exec));
  else
    exec(command, extend({ maxBuffer: 1000 * 1024 }, options.exec), cb);
